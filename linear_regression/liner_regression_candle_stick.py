...
# Criando um dataframe para o gráfico de candlestick
candlestick_df = df_completo[['Open', 'High', 'Low', 'Close', 'Volume']]
candlestick_df.index = df_completo.index

# Verificando se a coluna "Volume" existe
if 'Volume' not in candlestick_df.columns:
    raise ValueError('Column "Volume" NOT FOUND in Input DataFrame!')

# Plotando o gráfico de candlestick
mpf.plot(candlestick_df, type='candle', style='charles', title='Gráfico de Candlestick - TAEE4', ylabel='Preço', ylabel_lower='Volume', volume=True)

# Adicionando as previsões ao gráfico
fig, ax = plt.subplots()
plt.plot(df.index, df["real"], label="Real", color="blue", marker='o')
plt.plot(df.index, df["previsao"], label="Previsto", color="red")
plt.xlabel("Data")
plt.ylabel("Valor fechamento")
plt.legend()
plt.savefig("TAEE4.png")

# Calculando a diferença entre real e previsto
